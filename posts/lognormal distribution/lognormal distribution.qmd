...
Suppose the lifetime of an engine has a lognormal distribution. What is the probability that the lifetime will exceed 12,000 hours if the mean is 11 hours and the standard deviation is 1.3 hours?

```{r, fig.width=6, fig.height=8}
#| echo: false  

# Liftime of an engine

# Parameters
mean_lifetime <- 11      # mean of the underlying normal distribution
sd_lifetime <- 1.3       # standard deviation of the underlying normal distribution
threshold_lifetime <- 12000  # threshold value

# Calculate the probability that the lifetime exceeds the threshold
probability_exceeds_threshold <- 1 - pnorm(log(threshold_lifetime), mean = mean_lifetime, sd = sd_lifetime)
...
cdf_values <- plnorm(x_values, meanlog = log(mean_lifetime), sdlog = sd_lifetime)

# CDF Plot
plot(x_values, cdf_values, type = 'l', col = 'lightblue', lwd = 2,
     main = 'Engine liftime CDF',
     ylab = 'Cumulative Probability', xlab = 'Lifetime (hours)')

# Add a horizontal line at the threshold probability
abline(h = probability_exceeds_threshold, col = 'red', lty = 2)

# Add legend
legend("bottomright", legend = c("CDF", "Threshold"), col = c("lightblue", "red"), border = NA, lty = 1:2, cex = 0.8, bty = "n")
